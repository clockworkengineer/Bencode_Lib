...
TEST_CASE("ISource (Buffer interface). Contains file singlefile.torrent.",
  ...
  SECTION("Create BufferSource with empty buffer.", "[Bencode][ISource]") {
    REQUIRE_THROWS_AS(BufferSource(""), Bencode_Lib::Error);
    REQUIRE_THROWS_WITH(
        BufferSource(""),
        "Bencode Error: Empty source buffer passed to be decoded.");
  }
...
TEST_CASE("ISource (File interface).", "[Bencode][ISource]") {
  ...
  SECTION("Create FileSource with non existant file.", "[Bencode][ISource]") {
    ...
    REQUIRE_THROWS_WITH(FileSource(prefixTestDataPath(kNonExistantTorrent)),
                        "Bencode Error: Bencode file input stream failed to "
...
TEST_CASE("Use of BNode indexing operators", "[Bencode][BNode][Index]") {
  ...
  SECTION("Decode dictionary and check an invalid key generates exception",
    ...
    BufferSource bEncodeSource{
        "d3:one10:01234567895:three6:qwerty3:two9:asdfghjkle"};
    bEncode.decode(bEncodeSource);
    REQUIRE_THROWS_AS((bEncode.root())["onee"].is_dictionary(),
                      BNode::Error);
    REQUIRE_THROWS_WITH((bEncode.root())["onee"].is_dictionary(),
  ...
  SECTION("Decode list and check an invalid index generates exception",
    ...
    BufferSource bEncodeSource{
        "l6:sillyy12:poiuytrewqas26:abcdefghijklmnopqrstuvwxyze"};
    bEncode.decode(bEncodeSource);
    REQUIRE_THROWS_AS((bEncode.root())[3].is_list(),
                      BNode::Error);
...
TEST_CASE("Check BNode reference functions work.",
  ...
  SECTION("Integer reference.", "[Bencode][BNode][Reference]") {
    ...
    bEncode.decode(bEncodeSource);
    REQUIRE(BRef<Integer>((bEncode.root())).integer() == 45500);
  }
  SECTION("String reference.", "[Bencode][BNode][Reference]") {
    BufferSource bEncodeSource{"10:0123456789"};
    bEncode.decode(bEncodeSource);
    REQUIRE(BRef<String>((bEncode.root())).string() == "0123456789");
  }

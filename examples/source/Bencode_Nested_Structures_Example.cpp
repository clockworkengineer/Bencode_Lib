...
// Description: Demonstrates encoding and decoding nested lists and dictionaries with Bencode_Lib.
...
int main() {
    ...
    be::Bencode::DictionaryInitializerType dict = {
        ...
        {"roles", be::Bencode::ListInitializerType{
            std::string("admin"),
            std::string("editor")
        }},
        {"profile", be::Bencode::DictionaryInitializerType{
            {"age", 28},
            {"active", std::string("yes")}
        }}
    };
    be::Bencode doc(dict);
    ...
    std::cout << "Decoded user: " << be::NRef<be::String>(decoded["user"]).value() << std::endl;
    std::cout << "Decoded profile age: " << be::NRef<be::Integer>(decoded["profile"]["age"]).value() << std::endl;
    return 0;
}
